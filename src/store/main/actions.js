...
export default {
	...
	async getMainInfo(context) {
		...
		const dashEndPoint = "https://cooperative-deer-overalls.cyclic.app/api/v1/main/dashboard";

		// Get User Likes
		const userLikesEndPoint = "https://cooperative-deer-overalls.cyclic.app/api/v1/users/getLikes";

		// Get User Reacts (upvotes and downvots)
		const userReactsEndPoint = "https://cooperative-deer-overalls.cyclic.app/api/v1/posts/allReacts";

		// Get user's likes Info
		const favsEndPoint = "https://cooperative-deer-overalls.cyclic.app/api/v1/main/favourites";

		// Get the user Likes
		const userLikesResponse = await axios
			.get(userLikesEndPoint, { headers: { Authorization: `Bearer ${token}` } })
			.catch((error) => {
	...
	async getCoinInfo(context, payload) {
		...
		// Dashboard API Endpoint
		const coinInfoEndPoint = `https://cooperative-deer-overalls.cyclic.app/api/v1/main/coininfo/${payload}`;

		// Get the user Likes
		const coinInfoResponse = await axios
			.get(coinInfoEndPoint, { headers: { Authorization: `Bearer ${token}` } })
			.catch((error) => {
				console.log("*** ERROR 1 ***", error);
				throw new Error("Something went wrong while fetching data, Please try again later.");
			});
	...
	async addLike(context, payload) {
		...
		const addLikeEndPoint = `https://cooperative-deer-overalls.cyclic.app/api/v1/users/addLike/${payload.coinID}`;
		...
		// Send HTTP Request
		await axios.post(addLikeEndPoint, data, { headers: { Authorization: `Bearer ${token}` } }).catch((error) => {
			console.log("*** ERROR 1 ***", error);
	...
	async removeLike(context, payload) {
		...
		// Add Like API Endpoint
		const removeLikeEndPoint = `https://cooperative-deer-overalls.cyclic.app/api/v1/users/removeLike/${payload.coinID}`;

		// Send HTTP Reques
		await axios.get(removeLikeEndPoint, { headers: { Authorization: `Bearer ${token}` } }).catch((error) => {
			console.log("*** ERROR 1 ***", error);
	...
	async updateFavs(context) {
		...
		await axios
			.post(favsEndPoint, userLikes, { headers: { Authorization: `Bearer ${token}` } })
			.then((response) => {
	...
	async getSpecificUserData(context, payload) {
		...
		// Dashboard API Endpoint
		const userInfoEndPoint = `https://cooperative-deer-overalls.cyclic.app/api/v1/main/getUser/${payload}`;

		// Get the user Likes
		const userInfo = await axios.get(userInfoEndPoint, { headers: { Authorization: `Bearer ${token}` } }).catch((error) => {
			console.log("*** ERROR 1 ***", error);
			throw new Error("Something went wrong while fetching data, Please try again later.");
		});
	...
	async addReact(context, payload) {
		...
		const addReactEndPoint = `https://cooperative-deer-overalls.cyclic.app/api/v1/posts/addReact/${payload.postID}`;
		...
		// Send HTTP Request
		await axios.post(addReactEndPoint, data, { headers: { Authorization: `Bearer ${token}` } }).catch((error) => {
			console.log("*** ERROR 1 ***", error);
	...
	async updateReacts(context) {
		...
		await axios
			.get(userReactsEndPoint, { headers: { Authorization: `Bearer ${token}` } })
			.then((response) => {
